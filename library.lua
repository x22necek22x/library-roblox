...
function Library:Notify(title, text, duration)
    ...
    notificationFrame.Position = UDim2.new(0.5, -150, 0.7, #activeNotifications * 45)
    ...
    notificationFrame:TweenSize(UDim2.new(0, 300, 0, 40), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.3, true)
    
    task.delay(duration or 3, function()
        if notificationFrame then
            notificationFrame:TweenSize(UDim2.new(0, 0, 0, 40), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.3, true)
            task.wait(0.4)
...
function Library:AddExitButton(page)
    page:addButton("Exit Script", function()
        guiRunning = false
        local gui = game.CoreGui:FindFirstChild("fu8rj82n")
        if gui then gui:Destroy() end
    end)
end

function Library:ScriptShouldRun()
    return guiRunning
end
